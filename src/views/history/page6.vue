    ...
    <el-main class="main">
      <div  style="width: 100%; background-color: white; float: left;  border-radius: 5px; height: 89.4vh;"  v-loading="loading">
        <div class="tipBox">
          ...
          <div style="margin-left: 15px;font-size: 20px;font-weight: bold;margin-top: -4px;">闸门运行运行记录</div>
        </div>
...
export default {
  ...
  mounted() {
    ...
    //   }
    // });
    this.queryParams1.deviceId = '2d6d051613bb4762ac822b05f1a1337b';
    this.handleQuery1();
  ...
  methods:{
    getChildMsg(val){
      if (val>10){
        this.queryParams1.deviceId = this.gateList[val-11].value;
      }else {
        this.queryParams1.deviceId = '2d6d051613bb4762ac822b05f1a1337b';
      }
      this.handleQuery1();
    ...
    drawRightChart(obj){
      ...
      this.drawChart2('main6','开次次数','开次次数(次)',obj.data[3],obj.date,'#E62100');
    },
    handleChange(val){
      let array =val;
      for (let i = 1; i < 6; i++) {
